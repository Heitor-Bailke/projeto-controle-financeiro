...
O fluxo `Esqueceu a senha?` envia um link de uso único, válido por 1 hora, para o e-mail informado. A sessão usa cookies `HttpOnly`, `Secure` em produção e `SameSite=Strict`; os tokens não ficam acessíveis ao JavaScript. Para habilitar o envio real, preencha no `backend/.env` `FRONTEND_URL`, `SMTP_HOST`, `SMTP_PORT`, `SMTP_SECURE`, `SMTP_USER`, `SMTP_PASSWORD` e `SMTP_FROM`. Em desenvolvimento sem SMTP, o link é registrado no log do backend para teste local; nenhum e-mail é enviado.

O banco PostgreSQL cria a tabela `password_reset_tokens` automaticamente na inicialização. Em produção, use um provedor SMTP com TLS e mantenha as credenciais fora do controle de versão.

Após cinco falhas consecutivas de login para o mesmo identificador, novas tentativas ficam bloqueadas por 15 minutos. Eventos de sucesso, falha e bloqueio são registrados sem senha, token ou e-mail em claro.
